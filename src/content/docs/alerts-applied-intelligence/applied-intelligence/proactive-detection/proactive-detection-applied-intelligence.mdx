...
With Applied Intelligence's Anomaly Detection, anomalies from your [APM](/docs/apm)-monitored applications are automatically surfaced in our activity streams and anomalies feed. Each anomaly can be clicked on to bring up an automatic analysis. You can have notifications for anomalies delivered, via Destinations, to Slack, or you can set up a webhook to deliver messages where you need them. Anomalous events are available for querying, creating custom dashboards, and alerting. By creating an Anomaly Detection configuration (a group of apps you’re interested in), you can then add this configuration as a source, and have anomalies automatically correlated with other data sources via [Incident Intelligence](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence).
...
To view the Applied Intelligence anomalies feed:

1. From **[one.newrelic.com](https://one.newrelic.com)**, click **Alerts & AI**.
2. Under **Analyze**, click **Issues & activity**
3. Select the **Anomalies** tab
...
Anomaly Detection is enabled automatically, at no additional cost. To receive notifications or to have a configuration (group of apps) that you can add as a source for Incident Intelligence, you will need to create an Anomaly Detection configuration. You can create a configuration in the Alerts & AI UI:

1. From **[one.newrelic.com](https://one.newrelic.com)**, click **Alerts & AI**. 
2. Under **Detect**, click **Anomaly Detection**.
...
This leads to a section that does nothing but lead to another section 
6. Optional: [Correlate with other alerts](#add-source).
...
To find these settings: from **Alerts & AI**, under **Detect**, click **Anomaly Detection**. 
...
## Add anomalies as sources [#add-source]

When creating or editing an Anomaly configuration, you can select **Correlate with other alerts** to get context and correlations within Issues.

Alternatively, you can setup this correlation under Sources. To learn about doing this in Sources, see [Configure sources](/docs/alerts-applied-intelligence/applied-intelligence/incident-intelligence/get-started-incident-intelligence/#1-configure-sources).

## Webhook payload and examples [#webhook]

Anomaly Detection sends the event body in JSON format via HTTPS POST. The system expects the endpoint to return a successful HTTP code (2xx). If you use webhooks to configure Anomaly Detection, use these examples of the webhook body format and JSON schema.
